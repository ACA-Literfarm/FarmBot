...
class ChatSessionRepository(IChatSessionRepository):
    ...
    async def get_all_chats_by_user_id(self, litefarm_user_id: UUID) -> Sequence[ChatSession]:
        result = await self.db.execute(
    ...
    async def deactivate_all_chats_by_user_id(self, litefarm_user_id: UUID) -> None:
